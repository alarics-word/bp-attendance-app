...
/// Call `pullRoster()` on app start / whenever connectivity returns.
/// Call `pushAll()` whenever connectivity returns to flush the
/// offline queue (new manual-entry students, sessions, scans).
class SyncService {
  ...
  Future<void> _pushSessions() async {
    ...
    for (final s in sessions) {
      try {
        ...
        // sessions table has no synced flag — upsert is idempotent, safe to retry
      }
    }
  }
