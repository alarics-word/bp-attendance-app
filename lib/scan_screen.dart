...
class _ScanScreenState extends State<ScanScreen> {
  ...
  bool _cooldown = false;           // 1s guard against double-scanning
  String? _lastStatusMessage;
  Color _statusColor = Colors.grey;
  List<String> _recentScans = [];   // student_ids scanned this session, for the undo/list UI
  ...
  Future<void> _onDetect(BarcodeCapture capture) async {
    if (_cooldown) return; // ignore scans during cooldown window

    final code = capture.barcodes.first.rawValue;
    if (code == null) return;

    setState(() => _cooldown = true);
    // 1 second refresh/cooldown so the same physical scan isn't registered twice
    ...
    await _db.addAttendance(widget.sessionName, widget.date, code);
    setState(() {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              // Adds to local roster (unsynced) so this ID auto-matches on future scans,
              // and records the attendance flagged as manual.
              await _db.addLocalStudent(
                  scannedId, nameCtrl.text, deptCtrl.text, yearCtrl.text);
              await _db.addAttendance(widget.sessionName, widget.date, scannedId,
                  isManual: true);
              setState(() {
                _lastStatusMessage = '✅ ${nameCtrl.text} added & scanned (manual)';
                _statusColor = Colors.orange; // distinguish manual from roster-matched
